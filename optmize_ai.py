...
combined_metrics = pd.concat([
    kubevirt_metrics_numeric[['kubevirt_vmi_cpu_usage_seconds_total', 'kubevirt_vmi_memory_used_bytes']],
...
plt.title('Determining the Optimal Number of Groups', fontsize=16, color='#1976D2')  # Blue
...
plt.plot(elbow_point, wcss[elbow_point-1], marker='o', markersize=10, color='#D32F2F')  # Red
plt.annotate('Optimal number of groups identified here', 
             ...
             fontsize=12, color='#D32F2F')  # Red

# Add a brief explanation above the chart
plt.figtext(0.5, 0.9, "Using the Elbow Method to determine the optimal number of groups for clustering.",
            wrap=True, horizontalalignment='center', fontsize=14, color='#1976D2')  # Blue
...
underutilized_vms = combined_metrics[combined_metrics['Cluster'] == 2].shape[0]  # Assuming cluster 2 is underutilized
overutilized_vms = combined_metrics[combined_metrics['Cluster'] == 3].shape[0]  # Assuming cluster 3 is overutilized
optimal_vms = combined_metrics[combined_metrics['Cluster'] == 1].shape[0]  # Assuming cluster 1 is optimal
...
plt.title('Cost Savings from Optimal Resource Allocation', fontsize=16, color='#1976D2')  # Blue
...
current_usage = combined_metrics['kubevirt_vmi_cpu_usage_seconds_total'][:10]  # Example current usage
forecasted_usage = current_usage * 1.1  # Example forecast (10% increase)
plt.plot(time, current_usage, label='Current Usage', marker='o', color='#1976D2')  # Blue
plt.plot(time, forecasted_usage, label='Forecasted Usage', linestyle='--', marker='o', color='#F57C00')  # Orange
plt.title('Predictive Maintenance: Current vs. Forecasted Usage', fontsize=16, color='#1976D2')  # Blue
